...
import { getUserByEmail, uploadStorePackImageToStorage } from "@/lib/supabase";
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const uploaded = await uploadStorePackImageToStorage({
            fileName: file.name,
        ...
        return NextResponse.json(
            {
                ...
                message:
                    error instanceof Error
                        ? error.message
                        : "Erro ao enviar imagem do pacote para o Storage.",
            },
            { status: 500 },
        );
    }
