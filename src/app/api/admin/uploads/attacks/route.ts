...
import { getUserByEmail, uploadAttackImageToStorage } from "@/lib/supabase";
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const uploaded = await uploadAttackImageToStorage({
            fileName: file.name,
        ...
        return NextResponse.json(
            {
                success: false,
                file: null,
                message: error instanceof Error ? error.message : "Erro ao enviar imagem para o Storage.",
            },
            { status: 500 },
        );
    }
