...
type ImportLocationImagesResponseDto = {
    success: boolean;
    uploaded: number;
    updated: number;
    skipped: number;
...
async function ensureAdminBySessionEmail(email: string | null | undefined) {
    ...
    if (user.role !== "admin") {
        return {
            ok: false as const,
            status: 403,
            message: "Acesso negado. Apenas admin pode importar imagens de locais.",
        };
    }
...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!access.ok) {
            const denied: ImportLocationImagesResponseDto = {
                success: false,
                uploaded: 0,
                updated: 0,
                skipped: 0,
        ...
        if (files.length === 0) {
            const emptyResponse: ImportLocationImagesResponseDto = {
                success: false,
                uploaded: 0,
                updated: 0,
                skipped: 0,
        ...
        const locationByNormalizedName = new Map(
            locations.map((location) => [normalizeLookup(location.name), location]),
        );
        ...
        for (const file of files) {
            ...
            const locationNameFromSeed = seedFileNameMap.get(fileKey);
            const location = locationByNormalizedName.get(
                locationNameFromSeed ? normalizeLookup(locationNameFromSeed) : fileKey,
            );
            ...
            try {
                ...
                const uploadedFile = await uploadLocationImageToStorage({
                    fileName: file.name,
                    fileBuffer,
                ...
                uploaded += 1;
                await updateLocationImageFileById(location.id, uploadedFile.path);
                updated += 1;
            } catch (error) {
                skipped += 1;
                failedFiles.push({
        ...
        const response: ImportLocationImagesResponseDto = {
            success: true,
            uploaded,
            updated,
            skipped,
