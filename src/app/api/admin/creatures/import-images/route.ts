...
type ImportCreatureImagesResponseDto = {
    success: boolean;
    uploaded: number;
    updated: number;
    skipped: number;
...
async function ensureAdminBySessionEmail(email: string | null | undefined) {
    ...
    if (user.role !== "admin") {
        return {
            ok: false as const,
            status: 403,
            message: "Acesso negado. Apenas admin pode importar imagens de criaturas.",
        };
    }
...
function buildSeedFileNameMap(): Map<string, string> {
    ...
    for (const item of creaturesSeed as SeedCreature[]) {
        ...
        const seedFileName = typeof item.fileName === "string" ? item.fileName.trim() : "";
        if (!seedFileName) {
            continue;
        }
...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!access.ok) {
            const denied: ImportCreatureImagesResponseDto = {
                success: false,
                uploaded: 0,
                updated: 0,
                skipped: 0,
        ...
        if (files.length === 0) {
            const emptyResponse: ImportCreatureImagesResponseDto = {
                success: false,
                uploaded: 0,
                updated: 0,
                skipped: 0,
        ...
        const creatureByNormalizedName = new Map(
            creatures.map((creature) => [normalizeLookup(creature.name), creature]),
        );
        ...
        for (const file of files) {
            ...
            const creatureNameFromSeed = seedFileNameMap.get(fileKey);
            const creature = creatureByNormalizedName.get(
                creatureNameFromSeed ? normalizeLookup(creatureNameFromSeed) : fileKey,
            );
            ...
            try {
                ...
                const uploadedFile = await uploadCreatureImageToStorage({
                    fileName: file.name,
                    fileBuffer,
                ...
                uploaded += 1;
                await updateCreatureImageFileById(creature.id, uploadedFile.path);
                updated += 1;
            } catch (error) {
                skipped += 1;
                failedFiles.push({
        ...
        const response: ImportCreatureImagesResponseDto = {
            success: true,
            uploaded,
            updated,
            skipped,
