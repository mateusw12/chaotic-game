    ...
    for (const creature of creatures) {
      try {
        ...
        let matchingImage = images.find((img) => {
          ...
          return (
            imgName.includes(creature.fileName as string) ||
            (normalizedFileName && imgName.includes(normalizedFileName))
          );
        });

        if (!matchingImage) {
          console.warn(`[SYNC] Imagem não encontrada para: ${creature.name} (normalized: ${creature.fileName})`);
        ...
        const newPath = await renameCreatureImage(matchingImage.id, creature.fileName || creature.name);
        console.log(`[SYNC] Imagem renomeada de "${matchingImage.id}" para "${newPath}"`);

        // Atualiza o imageFileId da creature e o fileName
        await updateCreatureById(creature.id, {
          name: creature.name,
          fileName: creature.fileName, // Atualiza fileName com o valor de fileName
          rarity: creature.rarity,
          imageFileId: newPath,
          tribe: creature.tribe,
          power: creature.power,
