...
function mapRow(row: SupabaseAttackRow): AttackDto {
  ...
  return {
    ...
    baseDamage: row.base_damage ?? null,
...
function validatePayload(payload: CreateAttackRequestDto | UpdateAttackRequestDto) {
  ...
  if (payload.baseDamage != null) {
    const bd = Number(payload.baseDamage);
    if (Number.isNaN(bd) || bd < 0) {
      throw new Error("baseDamage inválido.");
    }
  }
...
export async function listAttacks(): Promise<AttackDto[]> {
  ...
  const withFileNameSelect = "id,name,file_name,rarity,image_file_id,image_url,energy_cost,base_damage,element_values,abilities,created_at,updated_at";
  const fallbackSelect = "id,name,rarity,image_file_id,image_url,energy_cost,base_damage,element_values,abilities,created_at,updated_at";
...
export async function createAttack(payload: CreateAttackRequestDto): Promise<AttackDto> {
  ...
  const insertPayload = {
    ...
    ...(payload.baseDamage !== undefined ? { base_damage: (payload.baseDamage === null ? null : Number(payload.baseDamage)) } : {}),
