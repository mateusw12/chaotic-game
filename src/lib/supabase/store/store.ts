...
function normalizeStorePackPayload(payload: CreateAdminStorePackRequestDto | UpdateAdminStorePackRequestDto) {
  ...
  return {
    ...
    weekly_limit: payload.weeklyLimit !== undefined && payload.weeklyLimit !== null
      ...
      : null,
    tags: Array.isArray((payload as any).tags) ? (payload as any).tags.filter(Boolean) : undefined,
...
export async function updateAdminStorePack(packId: string, payload: UpdateAdminStorePackRequestDto): Promise<AdminStorePackDto> {
  ...
  const { data, error } = await supabase
    ...
    .update(updateData)
    .eq("id", packId)
    .select("id,name,description,image_file_id,tags,image_url,cards_count,card_types,allowed_tribes,tribe_weights,rarity_weights,guaranteed_min_rarity,guaranteed_count,price_coins,price_diamonds,daily_limit,weekly_limit,is_active,created_at,updated_at")
    .single<SupabaseStorePackRow>();
